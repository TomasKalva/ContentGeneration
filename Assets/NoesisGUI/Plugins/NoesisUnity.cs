...
public class NoesisUnity
{
    ...
    private static void SetFontFallback()
    {
        if (_settings.defaultFont != null)
        {
            NoesisFontProvider.instance.Register(_settings.defaultFont);

            List<string> fonts = new List<string>();
            string baseUri = System.IO.Path.GetDirectoryName(_settings.defaultFont.source).Replace('\\', '/');
    ...
    public static void LoadApplicationResources()
    {
        try
        {
            if (_settings.applicationResources != null)
            {
                ...
                Noesis.GUI.LoadApplicationResources(_settings.applicationResources.source);
            }
        }
    ...
    #region Play Audio
    private static void PlayAudio(string filename, float volume)
    {
    ...
    private static MediaPlayer CreateMediaPlayer(MediaElement mediaElement, string uri, object user)
    {
        if (Application.isPlaying)
        {
            return new NoesisMediaPlayer(uri);
        }
    ...
    [DllImport(Library.Name)]
    static extern void Noesis_SetLicense(string name, string key);
